      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div ref={containerRef} className="relative mx-auto max-w-5xl">
          ...
          <ul className="space-y-12 md:space-y-16">
            {timeline.map((item, i) => {
              const alignRight = i % 2 === 0; // desktop: alternate sides
              return (
                <li key={item.id} className="relative">
                  ...
                  >
                    <span className={`relative grid h-9 w-9 place-items-center rounded-full border border-white/15 bg-ink-900 shadow-lg shadow-black/40`}>
                      <span className={`absolute inset-0 rounded-full bg-${item.accent}/15`} />
                      <item.icon
                        size={16}
                        className={`relative text-${item.accent}`}
                      />
                    </span>
                  </motion.div>
